/** * add.vue * Created by cc on 17/9/22. */
<template>
  <div>
    <div class="form-page">
...
import {
  ...
  get as contractGet
} from '../../api/PersonContractService';
...
export default {
  ...
  data() {
    return {
      objectName: '司机合同',
      configUrl: `${ApiConst.apiPersonDomain}/detail_render_info/contract/list`,
      domainObject: {},
      addFun: contractAdd,
      editFun: contractEdit,
      getInfo: contractGet,
      uploadUrl: ApiConst.apiPersonDomain,
      logType: 'personContract',
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (!this.domainObject.contractName) {
        this.$message({
          type: 'warning',
          message: '请查看合同名称！',
          duration: 3000
      ...
      if (this.domainObject.commission && !this.domainObject.commissionUnit) {
        this.$message({
          type: 'warning',
          message: '提成方式一栏填写不正确，请修改！',
          duration: 3000
        });
        return;
      }

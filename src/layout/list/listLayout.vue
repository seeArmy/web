      ...
      <div class="block">
        <div class="table-opr clearfix"  v-if="isAdvanced">
          <div class="opr-button fl">
            <!-- 这个才是导出，下面那个不知道干嘛的 -->
            <el-button v-if="showexportCsvBtn" type="default" @click="exportData" :disabled="btnDisabled">导出{{ objectName }}</el-button>

            <!-- 这个是Bob搞的导出,导出方法写在父组件上 -->
            <el-button v-if="showexportBtn" type="default" @click="exportCsvData" :disabled="exportDisabled" :loading="exportDisabled">导出{{ objectName }}</el-button>
            ...
            <el-button type="default" id="batch_approve_submit" v-show="showBatchPay" @click="handlebatchPay">批量付款</el-button>
            <el-button type="default" id="batch_set_withdraw" v-show="showBatchWithdrawBtn" @click="handlebatchWithdraw">批量设置提现额度</el-button>
        ...
        <v-table :selectable="selectable" :operatable="operatable" :getData="getData" :deleteFun="deleteFun" :editUrl="editUrl" :dispatchDelete="dispatchDelete" :actionUrl="actionUrl" :dispatch="dispatch" :dispatchUrl="dispatchUrl" :dispatchFreightSendCar="dispatchFreightSendCar" :actionLst="actionLst" :dispatchFreightSendCarUrl="dispatchFreightSendCarUrl" :columnsData="columnsData" :listData="listData" :searchData="searchData" :actionData="actionData" :operValues="operValues" :objectName="objectName" :uploadUrl="uploadUrl" v-loading.fullscreen.lock="loading" :refreshFreightFun="refreshFreightFun" :refreshFreightDispatch="refreshFreightDispatch" :closeFreightFun="closeFreightFun" :closeFreightDispatch="closeFreightDispatch" :pop-waybill-truck-map="popWaybillTruckMap"
        :pop-logsitcs-truck-map="popLogsitcsTruckMap" :batchSubmit="batchSubmit" :batchIgnored="batchIgnored">
        </v-table>
...
import { getParam, adaptHeight, getParamsFromURL, urlRedirect } from '../../../api/Utils';
...
export default {
  ...
  methods: {
    handlebatchWithdraw() {
      ...
      this.$emit('withdrawBatch', this.checkedBox)
    },
    handlebatchPay() {
      // console.log('thisdata', this.listData)
      //check
      if(this.checkedBox.length > 0) {
        for(var i in this.checkedBox) {
          if(this.checkedBox[0]['payerCode'] != this.checkedBox[i]['payerCode']) {
            this.$message({
              type: 'warning',
              message: '请选择同一个支付单位！',
              duration: 2000
    ...
    exportData() {
      ...
      if (dataTotal > 3000) {
        ...
        let paramObj = {};
        // 导出的条件 包含 extraParams 里面的 bug： 无车承运  单车付款审核 只是导出 委托模式
        if(this.extraParams && Object.keys(this.extraParams).length > 0) {
          Object.keys(this.extraParams).forEach((key) => {
            if(this.extraParams[key]) {
              paramObj[key] = this.extraParams[key];
            }
          })
        }
        Object.keys(this.searchModel).forEach((key) => {
          this.searchModel[key] ? (paramObj[key] = this.searchModel[key]) : '';
        });
        paramObj.page = 1;
        paramObj.size = dataTotal;
        this.loading = true;
        this.getColumn((success, error) => {
          ...
          this.exportCsvFunc(paramsObj, (success, error) => {
    ...
    exportCsvData() {
        ...
        this.$emit('exportMyCsv', paramObj)
    },
    exportCSV() {
    ...
    getData(from) {
      ...
      if(this.extraParams && Object.keys(this.extraParams).length > 0) {
        Object.keys(this.extraParams).forEach((key) => {
          if(self.extraParams[key]) {
            self.paramObj[key] = self.extraParams[key];
          }
        });
      }
      Object.keys(this.searchModel).forEach((key) => {
  ...
  created() {
    ...
    this.getSearch((searchConfig) => {
      if (searchConfig) {
        ...
        // 如果有searchModel 的默认值
        if(Object.keys(this.searchModelDefault).length>0){
          let defaultKeys = Object.keys(this.searchModelDefault);
          let searchModelKeys = Object.keys(this.searchModel);
          for(let i=0;i<defaultKeys.length;i++){
            let key1 = defaultKeys[i];
            let val1 = this.searchModelDefault[key1]
            for(let j=0;j<searchModelKeys.length;j++){
              let key2 = searchModelKeys[j];
              if(key1 == key2){
                self.$set(self.searchModel, key1, val1);
                break;
              }
            }
          }
  ...
  mounted() {
    ...
    function checkRow() {
      ...
      let countNum = 0;
      ckSingle.forEach((item,index) => {
        if (item.checked === true) {
          self.checkedBox.push(self.listData[index])
          countNum += 1;

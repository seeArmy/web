...
<template>
  <div id="template">
    ...
    <loginHeader :appInfo = 'appInfo' :isLoginPage = 'isLoginPage'></loginHeader>
    <!-- logreg page -->
    <div class="logreg">
      <!-- header -->
      <header class="header">
      ...
      <div class="logregfm">
          ...
          <div class="inner clearfix login-bg" :style="appInfo.background ? {backgroundImage: 'url(' + appInfo.background + ')'} : ''">
            ...
            <div class="regrec fr">
              <a href="/register/enterprise.html" class="el-button el-button--line" v-if="showRegister">注册与激活</a>
    ...
    <loginFooter :appInfo = 'appInfo' :isLoginPage = 'isLoginPage'></loginFooter>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loginCallback(response, err) {
      ...
      console.log('loginCallback ...', response, err, response.content);
      if (response.code === 200) {
        const resObj = response.content;
        if (resObj.loginAccount) {
          // 如果已经加入了公司组织
          if (resObj.orgCode) {
            const userInfo = {
            ...
            // 尚未加入公司组织
            window.location.href = `/register/join.html?loginAccount=${resObj.loginAccount}`;
          }
        } else {

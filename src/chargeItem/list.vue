  ...
  <div id="list" class="list-page">
    <div class="list-wrap">
      <div class="page-header clearfix">
        ...
        <div class="hd-opr fr">
          <a v-if="objectName === '费用科目配置' && !isAdvanced" href="javascript:;" :data-link="attachmentUrl" data-title="费用科目配置" class="el-button el-button--primary">查看更多</a>
          <a href="javascript:;" data-link="/chargeItem/add.html" data-title="新建费用科目配置" class="el-button el-button--primary"><i class="el-icon-plus"></i> 新建{{ objectName }}</a>
        </div>
      ...
      <div class="block">
        <v-table :selectable="false" :operatable="true" ref="table" @menuClick="menuClick" :editUrl="editUrl"
                 :columnsData="columnsData" :listData="listData" :searchData="searchData" :actionData="actionData"
                 :operValues="operValues" :objectName="objectName"
...
<script type="text/ecmascript-6">
  ...
  import { deleted, list, getColumn } from '../../api/ChargeItemService';
  ...
  export default {
    ...
    data() {
      return {
        ...
        leftDistance: '',
        topDistance: '',
        editUrl: `/chargeItem/add.html`,
        deleteFun: deleted,
        searchData: [],
        objectName: '费用科目配置',
        attachmentUrl: '/chargeItem/list.html',
    ...
    methods: {
      ...
      menuClick(command, menu, model) {
        switch (command) {
          case 'edit':
            this.openWindow(`${this.editUrl}&code=${model.code}&editable=true&objectName=${this.objectName}`, `编辑${this.objectName}`);
            break;
          case 'remove':
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              deleted(row.code, paramObj, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
                    message: '删除成功',
      ...
      getData() {
        ...
        this.editUrl = `/chargeItem/add.html?ownerOrgCode=${this.searchModel.ownerOrgCode}`;
      },

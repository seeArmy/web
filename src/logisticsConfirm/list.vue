...
<template>
  <div>
    <ele-list :selectable="selectable" :needAdd='false' :operatable="operatable" @query="query" @ready="query" @action="action" @more="more" @exportData="exportData" :total="total" :loading="loading" :list-data="listData" ref="eleList" :customColumn="customColumn" :params="params" :needPage="needPage" :isAdvanced="isAdvanced" :deleteFun="deleteFun" :add="add" :showExportButton='false' :downloadErrorData="downloadErrorData" :downloadTemplate="downloadTemplate" :importUrl="importUrl" :editUrl="editUrl" :getColumn="getColumn" :getList="getList" :exportCsv="exportCsv" :getSearch="getSearch" @uploadSuccess='uploadSuccess' :objectName="objectName" :showCustomBtn='false' customBtnName='批量确认订单' @customBtnClick='clickCustomBtn' :showImportButton='false' :showDownloadTemplate='false' :folder="folder">
    <span slot="funArea">
      <el-button @click="clickCustomBtn" v-check-permission="'logistics.batch.confirm.add'">批量确认订单</el-button>
      <el-button @click="assignMerchandiser" v-check-permission="'logisticsAssignPerson2.add'">分配跟单人</el-button>
    </span>
    </ele-list>
...
import { getColumnLogisticsConfirm as getColumn, getAction, confirmList as list, getSearchLogisticsConfirm as getSearch, deleted, exportCsvConfirm as exportCsv, add, downloadErrorData, downloadTemplate, addAction as executeAction, edit, reject, accept, assignMerchandisers } from '../../api/LogisticsService';
...
export default {
  ...
  data: () => {
    return {
      ...
      folder: 'logistics/logisticsConfirm',
      deleteFun: deleted,
      exportCsv,
      add,
      popTruckMap: true,
  ...
  methods: {
    ...
    doRejectAssign(actionCode, actionName, model) {
      const self = this;
      this.$msgbox({
        title: '确认',
        message: `确认拒绝成为该订单的跟单人吗？`,
        showCancelButton: true,
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true;
            instance.confirmButtonText = '执行中...';
            assignMerchandisers({ code: model.code }, (success, error) => {
              if (success) {
                this.$message({
                  type: 'success',
                  message: '拒绝成功',
    ...
    doSubmitSettle(actionCode, actionName, model) {
      const self = this,
        msg = actionCode === 'logisticsAccept' ? '确认要接单吗？' : '确认要拒绝接单吗？';
      this.$msgbox({
        title: '确认',
        message: `${msg}`,
        showCancelButton: true,
    ...
    action(command, menu, model) {
      ...
      switch (command) {
        ...
        case 'logisticsAccept':
          ...
          break;
        case 'rejectAssign':
          this.doRejectAssign(menu.key, menu.name, model);
          break;
        case 'map':
      ...
      list(searchParams, (success, error) => {
        ...
        if (success) {
          ...
          success.content.forEach((row) => {
            const actionList = [];
            if (checkPermission('logistics.confirm.add')) {
              actionList.push({ key: 'logisticsAccept', name: '确认接单' });
            }
            if (checkPermission('logistics.refuse.AssignPerson.add')) {
              actionList.push({ key: 'rejectAssign', name: '拒绝分配' });
            }
            if (checkPermission('logistics.refuse2.add')) {
              actionList.push({ key: 'logisticsReject', name: '拒绝接单' });
            }

...
(function () {
  ...
  LuShu.prototype.start = function () {
    this._map.removeEventListener("zoomend", this._handler);
    var me = this,
        len = me._path.length;
  ...
  LuShu.prototype.pause = function () {
    ...
    this._handler = function () {
      // const left =me._marker.point;
      if(me._marker.V){
        const top = me._marker.V.style.top;
        const left = me._marker.V.style.left;
        me._overlay._div.style.left = left;
      }
    };
    //标识是否是按过pause按钮
    this._fromPause = true;
    this._map.addEventListener("zoomend", this._handler);
    this._clearTimeout();
  };
  /**
   * 隐藏上方overlay
  ...
  LuShu.prototype.removeMaker = function () {
    this._removeMarker();
  };
  LuShu.prototype.addMarker = function () {
    this._addMarker();
  };
  /**
   * 显示上方overlay
  ...
  baidu.object.extend(LuShu.prototype, {
     ...
     * @return 无返回值.
     */
    _move: function (initPos, targetPos, effect, html) {
      var me = this,
          //当前的帧数
          currentCount = 0,
          //步长，米/秒
          ...
          times = this.times ? 10 / this.times : 10,
          pos;
      //如果小于1直接移动到下一点
      if (count < 1) {
        me._moveNext(++me.i);
    ...
    _moveNext: function (index) {
      ...
      if (index < this._path.length - 1) {
        me._move(me._path[index], me._path[index + 1], me._tween.linear, me.html);
      }else {
        me._setInfoWin(me.pos, me.html);
      }
      var event = new Event('moveNext');

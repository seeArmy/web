/** * list.vue 
* 订单分配列表 
* Created by cc on 19/3/8. 
*/
<template>
  <div>
    <ele-list :selectable="selectable" :needAdd='false' :operatable="operatable" @query="query" @ready="query" @action="action" @more="more" @exportData="exportData" :total="total" :loading="loading" :list-data="listData" ref="eleList" :customColumn="customColumn" :params="params" :needPage="needPage" :isAdvanced="isAdvanced" :deleteFun="deleteFun" :add="add" :downloadErrorData="downloadErrorData" :downloadTemplate="downloadTemplate" :importUrl="importUrl" :editUrl="editUrl" :getColumn="getColumn" :getList="getList" :exportCsv="exportCsv" :getSearch="getSearch" @uploadSuccess='uploadSuccess' :objectName="objectName" :showCustomBtn='false' customBtnName='批量确认订单' @customBtnClick='cilckCustomBtn' :showImportButton='false' :showDownloadTemplate='false' :folder="folder">
    <span slot="funArea">
      <el-button @click="assignMerchandiser" v-check-permission="'logisticsAssignPerson1.add'">分配跟单人</el-button>
    </span>
    </ele-list>
...
import { getColumnLogisticsAssign as getColumn, getAction, assignMerchandisersList as list, getSearchLogisticsAssign as getSearch, deleted, exportCsvAssign as exportCsv, add, downloadErrorData, downloadTemplate, addAction as executeAction, edit, reject, accept, assignMerchandisers } from '../../api/LogisticsService';
...
export default {
  ...
  data: () => {
    return {
      ...
      folder: 'logistics/logisticsAssign',
      deleteFun: deleted,
      exportCsv,
      add,
      popTruckMap: true,
  ...
  methods: {
    assignMerchandiser(type, model) {
      this.assignLogisticsCodes = [];
      let ckNum = 0;
      // 如果是列表单向操作
      if (type === 'single') {
        ckNum ++;
        this.assignLogisticsCodes.push(model.code);
      } else {
        // 批量操作
    ...
    action(command, menu, model) {
      ...
      switch (command) {
        ...
        case 'assignMerchandiser':
          this.assignMerchandiser('single', model);
          break;
      ...
      list(searchParams, (success, error) => {
        ...
        if (success) {
          ...
          success.content.forEach((data) => {
            const actionList = [];
             // v-check-permission="'logisticsAssignPerson1.add'"
            if (checkPermission('logisticsAssignPerson1.add')) {
              actionList.push({ key: 'assignMerchandiser', name: '分配跟单人' });
            }
            if (checkPermission('logistics.refuse1.add')) {

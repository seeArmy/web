...
<script type="text/ecmascript-6">
  ...
  import { certTruckSearch, certTruckColumn, certTruckList, certTruckRevokeApprove } from '../../../api/PlatformService';
  ...
  export default {
    ...
    data() {
      return {
        ...
        total: 0,
        loading: true,
        getColumn: certTruckColumn,
        getList: certTruckList,
        getSearch: certTruckSearch,
        objectName: '车辆认证',
        addUrl: '/platform/cert_truck/approval.html',
        editUrl: '/platform/cert_truck/approval.html',
    ...
    methods: {
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              certTruckRevokeApprove(row.truckCode, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
                    message: '操作成功',
      ...
      action(command, menu, model) {
        ...
        switch (command) {
          case 'verify':
            this.openWindow(`${this.editUrl}?code=${model.truckCode}&isShow=true`, `${this.objectName}`);
            break;
          case 'view':
            this.openWindow(`${this.editUrl}?code=${model.truckCode}&editable=false`, `${this.objectName}`);
            break;
          case 'reject':
            this.doReject(model);
            break;

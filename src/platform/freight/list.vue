...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable" :customAdd="true" :needAdd='false'
              ...
              :autoRefresh="true" :autoRefreshTimer="30"
...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, exportCsv, add, refresh, close, downloadErrorData, downloadTemplate, addAction as executeAction } from '../../../api/PlatformFreightService';
        ...
        list(searchParams, (success, error) => {
          ...
          if (success) {
            ...
            success.content.forEach((data) => {
              ...
              // 计算待处理车次 = 需求车次 -已接车次 - 已派车次 - 忽略车次
              data.pendingNumber = '';
              if(data.truckQty && data.acceptTruckNumber && data.ignoreTruckNumber && data.ignoreTruckNumber){
                  data.pendingNumber = data.truckQty - data.acceptTruckNumber - data.ignoreTruckNumber - data.ignoreTruckNumber ;
              }
            });

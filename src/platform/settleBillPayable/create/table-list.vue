<template>
  <div class="table-list-component fix-table-wrap settle-table" id="list">
    <el-table
      ...
      <el-table-column
        ...
        v-if="hiddenFields.indexOf(field.fieldConfigCode)===-1"
        :fixed="formatterFixedArrow(field.fieldConfigCode)"
        :key="index"
        min-width="160"
        :width="field.fieldConfigCode | filterWidth"
...
//    },
    ...
    filters:{
      //当时 订单号的时候，宽度增加
      filterWidth(fieldConfigCode){
        if(fieldConfigCode == 'logisticsCode'){
          return 200;
        }else{
          return '';
        }
    ...
    methods: {
      ...
      updateCurComponent(key, data) {
        // console.log('data[UnitCodeAsyncEdit`]', data[`${key}UnitCodeAsyncEdit`]);

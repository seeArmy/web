<template>
  <div>
    <ele-list :selectable="true" :operatable="true"
              ...
              :showBatchBtn1="showBatchBtn1" :showBatchBtn2="showBatchBtn2"
...
<script type="text/ecmascript-6">
  ...
  import { settlePayableList, settlePayableAllList, settlePayableListColumn, settlePayableListSearch, settlePayableApplyApprove, settlePayableApprove,
  settlePayableRevokeApplyApprove, settlePayableRevokeApprove, settlePayableCancel, settlePayableInvoice, settlePayableRevokeInvoice, settlePayableReceived , exportCsvPlatformBrokerBill, exportCsvBrokerBill, deleted } from '../../../api/PlatformSettleBillService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        objectName: localStorage.getItem('orgType') == '1' ? '应收客户账单' : '应付平台账单',
        editUrl: '/platform/settleBillPayable/create.html',
        recycleUrl: '/platform/settleBillPayable/recycleList.html',
        batchApplyNum: 0,
        batchApproveNum: 0,
        batchApproveList: [],
        batchApproveLabel: '',
    ...
    methods: {
      recycleList() {
        let tit = '';
        localStorage.getItem('orgType') == '1' ? (tit = '应收客户账单回收站列表') : (tit = '应付平台账单回收站列表');
        this.openWindow(`${this.recycleUrl}`, tit);
      },
      onReady() {
      ...
      action(command, menu, model) {
        switch (command) {
          ...
          // 编辑
          case 'edit':
            this.openWindow(`/platform/settleBillPayable/select_waybill.html?billCode=${model.code}&platformOrgCode=${model.platformPayerOrgCode}`, '编辑应收账单-选择运单');
            break;
          // 开票
          case 'invoice':
      ...
      doRemove(model) {
        ...
        this.$msgbox({
          title: '确认',
          message: `确认要删除${this.objectName} ${model.billNo} 吗？`,
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              this.deleted({ code: model.code }, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
                    message: '删除成功',
          ...
          if (success) {
            ...
            success.content.forEach((data) => {
              ...
              if (orgType !== "1") { //应付平台账单列表
                ...
                // 应付平台账单列表，账单增加删除功能（待提交审核、审核不通过、已作废三个状态才有删除操作）
                if (data.billStatus === 'none' || data.billStatus === 'reject' || data.billStatus === 'destroy') {

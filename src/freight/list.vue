...
<template>
  <div>
    ...
    <!-- 货源调整弹框 start =================================================== -->
    <freightManifest ref="freightManifest" @refresh="refresh"></freightManifest>
    <!-- 货源调整弹框  end =================================================== -->

    <qrcode-dialog ref="qrcodedialog" />
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, exportCsv, add, refresh, close, downloadErrorData, downloadTemplate, addAction as executeAction,getWaybillProgressByFreightCode } from '../../api/FreightService';
  import * as ApiConst from '../../api/ApiConst';
  import eleAction from '../components/ele-action/eleAction.vue';
  import freightManifest from './freightManifest.vue';
  import Decimal from "decimal.js";
  import QrcodeDialog from './components/qrcode-dialog.vue'
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        recycleUrl: '/freight/recycleList.html',
        deleteFun: deleted,
        exportCsv,
        add,
    ...
    methods: {
      ...
      recycleList() {
        this.openWindow(`${this.recycleUrl}`, '货源回收站列表');
      },
      ...
      action(command, menu, model) {
        ...
        switch (command) {
          ...
          case 'remove':
            this.doRemove(model);
            // this.$refs.freightManifest.open(model, command, menu.name);
            break;
          case 'editFreight':
            this.$refs.freightManifest.open(model, command, menu.name);
            break;
          case 'showQRcode':
            this.$refs['qrcodedialog'].showQRcode(model)
            break;
          default:
        ...
        list(searchParams, (success, error) => {
          ...
          if (success) {
            ...
            success.content.forEach((data) => {
              const actionList = [];
              // actionList.push({ key: 'delete', name: '删除' });
              ...
              // 货源列表增加货源调整控制权限
              if (checkPermission("freight.edit")) {
                actionList.push({ key: 'editFreight', name: '货源调整' });
              }
              actionList.push({ key: 'showQRcode', name: '查看二维码' });
              data.actionMenuList = actionList;
              // 计算待处理车次 = 已接车次  - 已派车次 - 忽略车次
              data.pendingNumber = '';

              let acceptTruckNumber = 0;
              if(data.acceptTruckNumber){
                acceptTruckNumber = data.acceptTruckNumber
              }

              let dispatchTruckNumber = 0;
              if(data.dispatchTruckNumber){
                dispatchTruckNumber = data.dispatchTruckNumber
              }

              let ignoreTruckNumber = 0;
              if(data.ignoreTruckNumber){
                ignoreTruckNumber = data.ignoreTruckNumber
              }

              data.pendingNumber = acceptTruckNumber - dispatchTruckNumber - ignoreTruckNumber;

              // 待处理车次小于0 则等于0
              if (data.pendingNumber < 0) {
                data.pendingNumber = 0;
              }

              // if(data.truckQty && data.acceptTruckNumber && data.ignoreTruckNumber && data.ignoreTruckNumber){
              //     data.pendingNumber = data.truckQty - data.ignoreTruckNumber - data.ignoreTruckNumber ;
              // }
            });

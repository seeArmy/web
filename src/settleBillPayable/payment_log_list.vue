  ...
  export default {
    ...
    data: () => {
      return {
        ...
        confirmFun: SettleBillService.settlePayableActualPaymentLogConfirm,
        ...
        importUrl: `${ApiConst.apiSettleDomain}/import/custom/actualPayment`,
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      doConfirm(model) {
        ...
        this.$msgbox({
          title: '确认',
          message: `您确认所选的付款资金流水${model.serialNumber}无误吗？`,
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              this.confirmFun(model.code, { code: model.code }, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
                    message: '确认成功',
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              instance.confirmButtonLoading = true;
              instance.confirmButtonText = '执行中...';
              this.deleteFun(model.code, { code: model.code }, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
                    message: '删除成功',
          ...
          if (success) {
            ...
            success.content.forEach((row) => {
              ...
              if (!row.confirmStatus) {
                actionList.push({ key: 'confirm', name: '确认' });
                actionList.push({ key: 'remove', name: '删除' });
              }
              row.actionMenuList = actionList;
            });
            console.log('this.codes', this.codes);
            this.codes.push('11'); //创造条件
            if (this.codes.length) {
              SettleBillService.settlePaymentRelationBillCount({
                paymentCodeList: this.codes.join(',')
              }, (res, err) => {
                if (err) {
                ...
                if (res) {
                  ...
                  success.content.forEach((item) => {
                    item.paymentNum = res.content[item.code] || '-';
                  });

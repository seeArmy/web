    ...
    <div class="list-wrap" v-show="!tips.status">
      ...
      <div>
        <h2 class="title-component">
          {{params.billCode && !readonly ? '编辑' : !readonly ? '创建' : ''}}应付账单{{readonly ? '详情' : ''}}</h2>
        ...
        <div v-if="readonly" class='tr'>
          ...
          <el-button @click="invoice" v-if="submitData.payableBillStatus==='accept' && !submitData.invoiced">记录开票</el-button>
      ...
      <div v-if="readonly">
        ...
        <!-- 付款记录 -->
        <payment-relation-list :submitData="submitData" @addReceiveDialog="addReceiveDialog" paramKey="billCode"></payment-relation-list>
      </div>
    </div>
    <!-- 表单页面，列表批量操作弹框 -->
  ...
  export default {
    ...
    data() {
      return {
        ...
        //            receivableAdjustmentQuotaAmount 应付调整定额总额
        //            receivableAdjustmentProportionAmount 应付调整比例金额总额
        //            taxReceivableFreightSubtotalAmount 含税应付运费小计总额
        //            receivableFreightSubtotalAmount 应付运费小计总额
        ...
        //          receivableEntireSingleAdjustment 应付整单调整
        ...
        submitData: {
          ...
          payeeOrgCode: '',
          payeeOrgName: '',
          payableAdjustmentProportionAmount: 0,
          payableAdjustmentQuotaAmount: 0,
          payableAmount: 0,
          payableEntireSingleAdjustment: 0,
          payableFreightSubtotalAmount: 0,
          payStatus: '',
          taxPayableFreightSubtotalAmount: 0,
          unreceivedAmount: 0,
          updateUsername: ''
        }
      };
    ...
    methods: {
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              SettleBillService.settlePayableReceivedInvoice(code, (success, error) => {
                if (error) {
                  this.$message({
                    type: 'error',
                    message: error.content,
      ...
      onGetList() {
        ...
        // this.searchData.carrierOrgName = this.searchData.carrierOrgName;
        // this.$delete(this.searchData, 'carrierOrgName');
        return new Promise((resolve, reject) => {
          /**
          ...
          SettleBillService[this.readonly ? 'settlePayableSelectContractWaybillList' : 'settlePayableSelectSelectedWaybillList'](this.searchData, (success, error) => {
            if (success) {
              success.content.forEach((item) => {
                // driverPrice 客户税率单位
                if (item.driverPrice && item.driverPriceUnitCode) {
                  this.carrierPriceData.forEach((uc) => {
                    if (uc.id === item.driverPriceUnitCode) {
                      item.driverPriceUnitCodeAsyncValue = uc.name;
                    }
                  });
                }
                // goodsLoss  合理货差系数
                ...
                if (item.goodsPrice && item.meterageType) {
                  ...
                  this.fields.forEach((field) => {
                    if (field.fieldConfigCode === 'meterageType') {
                      ...
                      const typeIndex = field.extraParams[0].optionsValue.indexOf(item.meterageType);
                      if (typeIndex !== -1) {
                        item.goodsPriceUnitCodeAsyncValue = field.extraParams[0].options[typeIndex];
                      }
                    }
                  });
                }
              ...
              console.log('onGetList:', success.content[0]);
       ...
       */
      onGetFields() {
        SettleBillService.settlePayableSelectedWaybillColumns(({
          code,
          content
        }) => {
          ...
          console.log('eleDefine content:', eleDefine);
          console.log('eleInfo content:', eleInfo);
          params = getParam({
            data: {
              content
            }

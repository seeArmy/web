    ...
    <div class="list-wrap" v-show="!tips.status">
      ...
      <div class="page-header clearfix">
        <h2 class="page-title fl">
          {{params.billCode && !readonly ? '编辑' : !readonly ? '创建' : ''}}应付账单{{readonly ? '详情' : ''}}
        </h2>
        <div class="hd-opr fr">
          <el-button v-if="!readonly" @click="onBack">返回到选择运单</el-button>
          <template v-if="readonly">
            <el-button @click="applyApprove" v-if="submitData.payableBillStatus==='none'">提交审核</el-button>
            <el-button @click="revokeApply" v-if="submitData.payableBillStatus==='pend'">撤回提交审核</el-button>
            <el-button @click="revokeConfirm" v-if="submitData.payableBillStatus==='pend'">审核账单</el-button>
            <!-- <el-button @click="revokeApprove" v-if="submitData.payableBillStatus==='accept'">撤回审核</el-button> -->
            <el-button @click="invoice" v-if="submitData.payableBillStatus==='accept' && submitData.receivedInvoice !== 'yes'">记录收票</el-button>
            <!-- <el-button @click="sendBill" v-if="submitData.payableBillStatus==='accept' && submitData.billAffirmStatus === 'none'">发送账单</el-button>
            <el-button @click="destroy" v-if="submitData.payableBillStatus==='none' || submitData.payableBillStatus==='reject'">作废账单</el-button>
            <el-button @click='onEditForm' v-if="submitData.payableBillStatus==='none' || submitData.payableBillStatus==='reject'">编辑账单</el-button> -->
            <!-- <el-button @click="addReceiveDialog" v-if="submitData.payableBillStatus==='accept' && submitData.payStatus!=='all'">添加付款</el-button> -->
          </template>
        </div>
...
<!--         <div class="search-form">
      ...
      <el-tabs type="border-card" v-if="readonly">
        ...
        <el-tab-pane label="进项发票记录">
          <invoice :code="code" :domainObject="submitData" v-if="invoiceShow"></invoice>
        </el-tab-pane>
        <el-tab-pane label="付款资金流水">
          <!-- <div class='tr'>
            <el-button @click="addReceiveDialog" v-if="submitData.payableBillStatus==='accept' && submitData.payableStatus!=='all'">添加付款</el-button>
          </div> -->
          <!-- 应付 -->
          <payment-relation-list paramKey="billCode" :submitData='submitData'></payment-relation-list>
        </el-tab-pane>
...
//  应付列表
import PaymentRelationList from './payment_relation_list.vue';
// 审核弹框确认
...
export default {
  ...
  data() {
    return {
      ...
      isMini: true,
      searchFields: [],
      folder: 'settle/payableSelWaybill',
      carrierPriceData: [],
      goodsPriceData: [],
      goodsLossData: [],
      chargeItemNumberData: [],
      history: {
        actualPaymentLogList: {
          list: []
        },
        approveList: {
          list: []
            ...
            beforeClose: (action, instance, done) => {
              if (action === 'confirm') {
                this.tableCellActions.remove({
                  codeList: data.code,
                  carrierOrgName: data.carrierOrgName,
                  billCode: ''
                }, ({
                  code
                } = {}) => {
      ...
      formatterFields: {
        ...
        // adjustmentQuota  "应付调整定额"
        adjustmentQuota(value, field) {
          return `￥${value[field.fieldConfigCode]}`;
        },
        // adjustmentProportion "应付调整比例"
        adjustmentProportion(value, field) {
          return `${value[field.fieldConfigCode]}%`;
        },
        // "adjustmentProportionAmount" "应付调整比例金额"
        adjustmentProportionAmount(value, field) {
          return `￥${value[field.fieldConfigCode]}`;
        },
        // "freightSubtotal"  "应付运费小计"
      ...
      //            adjustmentQuotaAmount 应付调整定额总额
      //            adjustmentProportionAmount 应付调整比例金额总额
      //            taxFreightSubtotalAmount 含税应付运费小计总额
      //            freightSubtotalAmount 应付运费小计总额
      //        billName (required) 账单名称
      //        payeeOrgCode (required) 付款方组织编码
      //        payeeOrgName (required) 付款方名称
      //          adjustmentWay 调整方式 quota 按整单定额调整 scale 按整单比例调整
      //          entireSingleAdjustmentRoportion 整单调整比例
      //          entireSingleAdjustment 应付整单调整
  ...
  methods: {
    ...
    onTableActionBtnClick({
      ...
      if (btn.key !== 'export') {
        ...
        if (this.params.billCode) {
          ...
          params.carrierOrgName = this.params.carrierOrgName;
        }
        // 获取列表字段配置
        SettleBillService.settlePayableSelectedWaybillColumns((success) => {
          if (success) {
            ...
            // 导出
            SettleBillService.settlePayableSelWaybillExportCsv(paramsObj, (success, error) => {
              if (error) {
    ...
    onTableCellSave({
      ...
      if (this.params.billCode) {
        ...
        this.updateParams.carrierOrgName = this.params.carrierOrgName;
      }
    ...
    addReceive() {
      // console.log('添加付款弹框点击确定', this.receive_dialog.data);
      const params = {
        billCode: this.receive_dialog.data.billCode,
        carrierOrgCode: this.receive_dialog.data.carrierOrgCode,
        carrierOrgName: this.receive_dialog.data.carrierOrgName,
      ...
      SettleBillService.settlePayableActualPaymentLogAdd(params, (success, error) => {
        if (success) {
          this.$message({
            type: 'success',
            message: '保存成功！',
          ...
          if (action === 'confirm') {
            ...
            SettleBillService.settlePayableInvoice(code, (success, error) => {
              ...
              if (success) {
                ...
                this.submitData.receivedInvoice = 'yes';
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            SettleBillService.settlePayableApprove(this.params.billCode, {
              ...
              if (success) {
                this.submitData.payableBillStatus = 'accept';
                this.$message({
                  type: 'success',
                  message: '审核通过成功！',
                  duration: 1000
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            SettleBillService.settlePayableApprove(this.params.billCode, {
              ...
              if (success) {
                this.submitData.payableBillStatus = 'reject';
                this.$message({
                  type: 'success',
                  message: '审核不通过操作成功！',
                  duration: 1000
          ...
          if (action === 'confirm') {
            ...
            SettleBillService.settlePayableRevokeApprove(code, (success, error) => {
              ...
              if (success) {
                ...
                this.submitData.payableBillStatus = 'pend';
                instance.confirmButtonLoading = false;
                done();
              }
              // 操作完毕刷新
          ...
          if (action === 'confirm') {
            ...
            SettleBillService.settlePayableRevokeApplyApprove(code, (success, error) => {
              ...
              if (success) {
                ...
                this.submitData.payableBillStatus = 'none';
                instance.confirmButtonLoading = false;
                done();
              }
              // 操作完毕刷新
    ...
    onBack() {
      // window.history.back();
      const path = '/settleBillPayable/select_waybill.html?',
        query = this.params.billCode ? `billCode=${this.params.billCode}` : `orgCode=${this.params.carrierOrgCode}&orgName=${this.params.carrierOrgName}`;
    ...
    onEditForm() {
      ...
      // this.readonly = false;
      window.location.href = `/settleBillPayable/select_waybill.html?billCode=${this.params.billCode}`;
    },
    ...
    onSubmit() {
      ...
      return new Promise((resolve, reject) => {
        if (!this.params.billCode) {
          ...
          delete tempData.billNo;      // ps: 此处应该 需要 billNo ，但是此处 后端已经修复，暂时不改
          ...
          SettleBillService.settlePayableEdit(this.params.billCode, tempData, (success, error) => {
            if (success) {
              this.tips.status = true;
              this.tips.type = 'edit';
    ...
    //      onGetActualPaymentLogList() {
    //        SettleBillService.settlePayableActualPaymentLogList({
    ...
    //            }) => {
    //          if (code === 200) {
    //            console.log('create_history->onGetActualPaymentLogList', content);
    //            this.history.actualPaymentLogList.list = content;
    //          } else {
    //            this.$message.error(content);
    //          }
    //        });
    ...
    onUpdateParams() {
      ...
      if (paramsObj.code) {
        ...
        this.onGetView().then((content) => {
          ...
          this.params.carrierOrgName = this.submitData.payeeOrgName;
          this.params.carrierOrgCode = this.submitData.payeeOrgCode;
          this.params.platformCarrierOrgCode = this.submitData.platformPayeeOrgCode;
        ...
        this.readonly = false;
        this.params.carrierOrgCode = paramsObj.carrierOrgCode;
        this.params.platformCarrierOrgCode = paramsObj.platformCarrierOrgCode;
        this.params.carrierOrgName = paramsObj.carrierOrgName;
        this.submitData.billName = this.params.carrierOrgName + date.getFullYear() + (date.getMonth() + 1) + date.getDate();
        this.submitData.payeeOrgName = this.params.carrierOrgName;
        this.submitData.payeeOrgCode = this.params.carrierOrgCode;
        this.submitData.platformPayeeOrgCode = this.params.platformCarrierOrgCode;
      }
      if (paramsObj.editable) {
        this.readonly = true;
        //          this.onGetActualPaymentLogList();
        this.onGetApproveList();
      }

...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getDispatchingColumn"
              :getList="getList"
              :exportCsv="exportJobOrderInfoCsv"
              :getSearch="getDispatchingSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getDispatchingColumn, getDispatchingSearch, exportJobOrderInfoCsv, downloadErrorData, downloadTemplate,getDispatchingList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        total: 0,
        loading: true,
        getDispatchingColumn,
        getList: getDispatchingList,
        getDispatchingSearch,
        objectName: '派车单信息',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/dispatchingList.html',
        exportJobOrderInfoCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportJobOrderInfoList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportJobOrderInfoCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getDispatchingList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

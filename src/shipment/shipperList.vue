...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getShipperColumn"
              :getList="getList"
              :exportCsv="exportShipperCsv"
              :getSearch="getShipperSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getShipperColumn, getShipperSearch, exportShipperCsv, downloadErrorData, downloadTemplate,getShipperList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        loading: true,
        getShipperColumn,
        getList: getShipperList,
        getShipperSearch,
        objectName: '托运人',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/shipperList.html',
        exportShipperCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportShipperInfoList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportShipperCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getShipperList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,
...
<style scoped>
.list-page .table-opr .complex-control-3 .el-input{
  width: 100% !important;
}
</style>

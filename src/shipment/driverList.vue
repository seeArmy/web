...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getDriverInfoColumn"
              :getList="getList"
              :exportCsv="exportDriverCsv"
              :getSearch="getDriverInfoSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getDriverInfoColumn, getDriverInfoSearch, exportDriverCsv, downloadErrorData, downloadTemplate,getDriverInfoList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        loading: true,
        getDriverInfoColumn,
        getList: getDriverInfoList,
        getDriverInfoSearch,
        objectName: '驾驶员基础信息',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/driverList.html',
        exportDriverCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportDriverInfoList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportDriverCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getDriverInfoList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

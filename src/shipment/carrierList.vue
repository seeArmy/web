...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getCarrierColumn"
              :getList="getList"
              :exportCsv="exportCarrierInfoCsv"
              :getSearch="getCarrierSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getCarrierColumn, getCarrierSearch, exportCarrierInfoCsv,getCarrierList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        loading: true,
        getCarrierColumn,
        getList: getCarrierList,
        getCarrierSearch,
        objectName: '承运人',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/carrierList.html',
        exportCarrierInfoCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportCarrierInfoList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportCarrierInfoCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getCarrierList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

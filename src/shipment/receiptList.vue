...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getReceiptColumn"
              :getList="getList"
              :exportCsv="exportReceiptInfoCsv"
              :getSearch="getReceiptSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getReceiptColumn, getReceiptSearch, exportReceiptInfoCsv, downloadErrorData, downloadTemplate,getReceiptList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        loading: true,
        getReceiptColumn,
        getList: getReceiptList,
        getReceiptSearch,
        objectName: '回执单信息',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/receiptList.html',
        exportReceiptInfoCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportReceiptInfoList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportReceiptInfoCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getReceiptList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

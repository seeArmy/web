...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getCapitalColumn"
              :getList="getList"
              :exportCsv="exportCapitalFlowCsv"
              :getSearch="getCapitalSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getCapitalColumn, getCapitalSearch, exportCapitalFlowCsv, downloadErrorData, downloadTemplate,getCapitalList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        loading: true,
        getCapitalColumn,
        getList: getCapitalList,
        getCapitalSearch,
        objectName: '资金流水',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/capitalList.html',
        exportCapitalFlowCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportCapitalFlowList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportCapitalFlowCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getCapitalList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

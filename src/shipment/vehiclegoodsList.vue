...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getVehiclegoodsColumn"
              :getList="getList"
              :exportCsv="exportGoodsOfVehicleCsv"
              :getSearch="getVehiclegoodsSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getVehiclegoodsColumn, getVehiclegoodsSearch, exportGoodsOfVehicleCsv, downloadErrorData, downloadTemplate,getVehiclegoodsList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        total: 0,
        loading: true,
        getVehiclegoodsColumn,
        getList: getVehiclegoodsList,
        getVehiclegoodsSearch,
        objectName: '车载货物信息',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/vehiclegoodsList.html',
        exportGoodsOfVehicleCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportGoodsOfVehicleList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportGoodsOfVehicleCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getVehiclegoodsList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getWaybillColumn"
              :getList="getList"
              :exportCsv="exportShippingNoteInfoCsv"
              :getSearch="getWaybillSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getWaybillList, getWaybillSearch, exportShippingNoteInfoCsv, downloadErrorData, downloadTemplate,getWaybillColumn } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        loading: true,
        getWaybillColumn,
        getList: getWaybillList,
        getWaybillSearch,
        objectName: '运单',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/waybillList.html',
        exportShippingNoteInfoCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportShippingNoteInfoList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportShippingNoteInfoCsv(params, (success, error) => {

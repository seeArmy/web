...
<template>
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
              ...
              :importUrl="importUrl"
              :editUrl="editUrl"
              :getColumn="getVehicebaseColumn"
              :getList="getList"
              :exportCsv="exportVehicleInfoCsv"
              :getSearch="getVehicebaseSearch"
...
<script type="text/ecmascript-6">
  ...
  import { getVehicebaseColumn, getVehicebaseSearch, exportVehicleInfoCsv, downloadErrorData, downloadTemplate,getVehicebaseList } from '../../api/ShipmentService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        loading: true,
        getVehicebaseColumn,
        getList: getVehicebaseList,
        getVehicebaseSearch,
        objectName: '车辆基础信息',
        addUrl: '',
        editUrl: '',
        listUrl: '/shipment/vehicebaseList.html',
        exportVehicleInfoCsv,
        importUrl: ''
      };
    },
    methods: {
      handleExportMyCsv(params){
       let url = `${ApiConst.apiShipmentDomain}/broker/js/export/exportVehicleInfoList?`
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          exportVehicleInfoCsv(params, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        this.listData = [];
        getVehicebaseList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, exportCsv, add, downloadErrorData, downloadTemplate, getDetail } from '../../api/TyreInspectionService';
  import * as ApiConst from '../../api/ApiConst';

  export default {
    name: 'tyreInspectiontList',
    components: {
      'ele-list': eleList
    },
    props: {
    ...
    data: () => {
      return {
        ...
        getSearch,
        objectName: '轮胎巡检记录',
        addUrl: '/tyre_inspection/add.html',
        editUrl: '/tyre_inspection/add.html',
        listUrl: '/tyre_inspection/list.html',
        deleteFun: deleted,
        exportCsv,
        add,
        downloadErrorData,
        importUrl: `${ApiConst.apiMaintenaceDomain}/import/custom/tyreInspection`,
        downloadTemplate
      };
    },
    methods: {
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      doRemove(model) {
        ...
        this.$msgbox({
          title: '确认',
          message: `确认要删除${this.objectName} ${model.tyreNo} 吗？`,

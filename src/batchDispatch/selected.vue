      ...
      <table class="rl-fix-table table-bordered">
        <thead>
          <tr>
            ...
            <th class="fix-hd">挂车牌号</th>
            <th class="fix-hd">车次数</th>
            <th class="fix-hd">主驾司机</th>
            <th class="fix-hd">副驾司机</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(list,index) in selData" :key="index" :class="{active : list.isChecked}" @click="selectRow(list)" v-if="selData.length > 0">
            ...
            <td>
              {{ list.mainTruckLicenseNo }}
            </td>
            <td>
              {{ list.trailerTruckLicenseNo }}
            </td>
            <td>
              <el-input-number size="small" v-model="list.countNumber" :min="1" @change="handleChange"></el-input-number>
            </td>
            <td>
              {{ list.driverFullName }}
            </td>
            <td>
              {{ list.viceDriverFullName }}
            </td>
          </tr>
          <tr v-else>
            <td colspan="6">
              <p>当前已选车辆为空，请在左侧待选车辆中选择车辆。</p>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    clearSelected() {
      ...
      this.$msgbox({
        title: '消息',
        message: h('p', null, [h('span', null, '确认清空当前全部已选车辆吗？ ')]),

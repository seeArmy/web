    ...
    <div class="el-col el-col-6">
      <div class="el-form-item">
        ...
        <div class="el-form-item__content" style="margin-left: 100px;">
          ...
          <a href="javascript:;" v-show="changeStatus1 && driver.code && driver.name" @click="openWindow(`${editUrl}?code=${driver.code}&editable=false`, `${objectName}详情`)" class="blue" :data-link="`${editUrl}?code=${driver.code}&editable=false`" :data-title="`${objectName}详情`" :title="`查看${objectName}详情`">
            {{ driver.name }}
          </a>
          <span v-show="changeStatus1 && !driver.code && driver.name">
            {{ driver.name }}
          </span>
          <div class="add" v-if="!driver.code && !driver.name">
            <span @click="addDriver('driver')" class="btn-link">添加</span>
          </div>
          <div class="change" v-if="driver.code || driver.name">
    ...
    <div class="el-col el-col-6">
      <div class="el-form-item">
        ...
        <div class="el-form-item__content" style="margin-left: 100px;">
          ...
          <a href="javascript:;" v-show="changeStatus2 && viceDriver.code && viceDriver.name" @click="openWindow(`${editUrl}?code=${viceDriver.code}&editable=false`, `${objectName}详情`)" class="blue" :data-link="`${editUrl}?code=${viceDriver.code}&editable=false`" :data-title="`${objectName}详情`" :title="`查看${objectName}详情`">
            {{ viceDriver.name }}
          </a>
          <span v-show="changeStatus1 && !viceDriver.code && viceDriver.name">
            {{ driver.name }}
          </span>
          <div class="add" v-if="!viceDriver.code && !viceDriver.name">
            <span @click="addDriver('viceDriver')" class="btn-link">添加</span>
          </div>
          <div class="change" v-if="viceDriver.code || viceDriver.name">
    ...
    <el-dialog title="请选择司机" :visible.sync="dialogTableVisible" size="large">
      <el-form :model="searchModel" ref="searchModel" label-width="140px" :inline="true" class="text-r">
        ...
        <el-form-item label="司机姓名">
          <el-input v-model="param.fullName" placeholder="请输入司机姓名"></el-input>
        </el-form-item>
        ...
        <table class="rl-fix-table table-bordered">
          ...
          <tbody>
            <tr v-for="(list, index) in listData" :key="index">
              <td class="fix-col">
                <el-button class="el-button--line" @click="setData(list)">选择</el-button>
                <!-- <el-button type="primary">删除</el-button> -->
              </td>
              <td v-for="column in columnsData" :key="column.fieldConfigCode">
                <span v-for="field in column.extraParams">
                {{ list[field.field] }}
                </span>
              </td>
            </tr>
...
export default {
  ...
  methods: {
    ...
    deletDriver(target) {
      ...
      this[target].phone = '';
      this[target].code = '';
      this[target].bankAccountNo = '';
      const params = {};
      if (target === 'driver') {
        params.driverFullName = '';
        params.driverCode = '';
        params.driverPhone = '';
        params.driverPayeeBankAccountNo = '';
      }
      if (target === 'viceDriver') {
        params.viceDriverFullName = '';
        params.viceDriverCode = '';
        params.viceDriverPhone = '';
        params.viceDriverPayeeBankAccountNo = '';
      }
    ...
    setData(list) {
      ...
      this[this.target].bankAccountNo = list.bankAccountNo;
      const params = {};
      if (this.target === 'driver') {
        ...
        params.driverPayeeBankAccountNo = this[this.target].bankAccountNo;
      }
      if (this.target === 'viceDriver') {
        params.viceDriverFullName = this[this.target].name;
        params.viceDriverCode = this[this.target].code;
        params.viceDriverPhone = this[this.target].phone;
        params.viceDriverPayeeBankAccountNo = this[this.target].bankAccountNo;
      }
      this.updateTransport(params);
      console.log(list);
    },
    ...
    getList() {
      ...
      DataSourceService[this.popData.datasource].getListData(this.param, (success) => {
        if (success) {
          this.itemData = success.content;
          success.content.forEach((val) => {
            val.certStatus = this.getText(val.certStatus);
          });
...
//          }
    ...
    getColumns() {
      DataSourceService[this.popData.datasource].getColumns((data) => {
        const localStorage = window.localStorage,
          ...
          paramData = getParam({ data }, eleDefine, eleInfo);
        console.log('columnsData', data, paramData.field);
        this.columnsData = paramData.field;
      });
    },
    getSearch() {
      const localStorage = window.localStorage,
  ...
  created() {
    ...
    const code = getParamsFromURL(window.location.search).code || this.seqCode,
      baseParams = {
        page: 1,
        size: 1
      },
    ...
    transportList(params, (data) => {
      if (data.content.length) {
        ...
        if (result.driverFullName) {
          ...
          this.driver.bankAccountNo = result.bankAccountNo;
        }
        if (result.viceDriverFullName) {
          this.changeStatus2 = true;
          this.viceDriver.name = result.viceDriverFullName;
          this.viceDriver.phone = result.viceDriverPhone;
          this.viceDriver.code = result.viceDriverCode;
          this.viceDriver.bankAccountNo = result.bankAccountNo;
        }
      }
    });
  },

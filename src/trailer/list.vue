...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, exportCsv, add, downloadErrorData, downloadTemplate ,getDetail } from '../../api/TrailerService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        getSearch,
        objectName: '挂车',
        addUrl: '/trailer/add.html',
        editUrl: '/trailer/add.html',
        listUrl: '/trailer/list.html',
        deleteFun: deleted,
        exportCsv,
        add,
        downloadErrorData,
        importUrl: `${ApiConst.apiOrgDomain}/import/custom/trailer`,
        downloadTemplate,
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      doRemove(model) {
        ...
        this.$msgbox({
          title: '确认',
          message: `确认要删除${this.objectName} ${model.truckLicenseNo} 吗？`,

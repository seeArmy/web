...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, blacklist, exportCsv, add, downloadErrorData, downloadTemplate } from '../../api/DriverMemberService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        getSearch,
        objectName: '司机会员',
        addUrl: '/driverManagement/add.html',
        editUrl: '/driverManagement/add.html',
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      doIdPrint(row) {
        ...
        let url = `/printDoc/id.html?userCode=${row.code}`;
        window.open(url);
      },
        ...
        list(searchParams, (success, error) => {
          ...
          if (success) {
            ...
            success.content.forEach((data) => {
              ...
              // 新增打印身份证
              actionList.push({ key: 'idPrint', name: '身份证打印' });

...
import { getMoneyFlowForCore as cashflowLst } from '../../api/virtualAccount'
  ...
  export default {
    ...
    methods: {
      handleExportCsv(params) {
        ...
        const api = 'pay/virtualAccount/export/moneyFlowForCore'
    ...
    created() {
      const appInfo = window.localStorage.getItem('appInfo')
      if(appInfo) {
        this.extraParams.code = JSON.parse(appInfo).coreOrgCode
      }

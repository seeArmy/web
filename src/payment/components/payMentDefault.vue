  ...
  <div id="driverPayment" class="form-page">
    ...
    <el-row v-if="confirmPay">
      <div class="block block-box">
        ...
        <h2>支付单位: <big>{{ nowPayeeName }}</big> (实际可用余额：<big class="page-title"> {{ totalAmount | toThousandslsFilter }} 元</big>)</h2>
...
export default {
  ...
  methods: {
    loadingTotalAmount() {
      ...
      const param = {
        code: o['platformPayerCode']
      }
      getCompanyAcountInfo(param, (success, err) => {
        if (success && success.code === 200) {
          const { content } = success;
          this.totalAmount = content.amount || 0
    ...
    handleconfirmPay() {
      var _totalMount = new Decimal(this.totalAmount).plus(0).toNumber()
      if(_totalMount < this.totalPay) {
        this.$message({
          type: 'error',
          message: '余额不足，无法支付',
          duration: 3000

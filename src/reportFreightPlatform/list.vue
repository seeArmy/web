...
*/
<template>
  <div id="pageReportFreightPlatform">
    <ele-list :selectable="selectable" :operatable="operatable"
      ...
      <template slot="funArea">
        <el-button type="default" @click="batchReportFreight" v-check-permission="'ReportedCapitalflowPlatform.get'">批量上报</el-button>
        <el-button type="default" @click="batchValidateFreight" v-check-permission="'CheckCapitalflowPlatform.get'">批量校验</el-button>
        <el-button type="default" @click="batchDeleteFreight" v-check-permission="'uploadCapitalflowPlatform.delete'">批量删除</el-button>
      </template>
    </ele-list>
...
<script type="text/ecmascript-6">
  ...
  import { getColumn,listPlatform as list,getSearch, downloadTemplate,deleteList, getAction, executeAction, exportCsv, add, downloadErrorData,reportFreight,validateFreight ,getDetail } from '../../api/ReportFreightService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        getSearch,
        objectName: '上报运费记录',
        addUrl: '/reportFreightPlatform/add.html',
        editUrl: '/reportFreightPlatform/add.html',
        listUrl: '/reportFreightPlatform/list.html',
        actionUrl: 'reportFreightPlatform/action.html',
          ...
          case 'validateFreight':
            this.$confirm('确认校验本运单吗？', '提示', {confirmButtonText: '确定',cancelButtonText: '取消',type: 'warning'}).then(() => {
              ...
              validateFreight({ code: model.code },(ret,err)=>{
                this.loading=false;
                if(ret && ret.content  && true == ret.content.result)  this.$message({type: 'success',  message: '校验成功！'});
                else if(ret && ret.content  && false == ret.content.result) this.$message({type: 'warning',  message: '校验失败！'});
                setTimeout(() => {
                  window.location.reload();
                }, 1000);
              })
        ...
        list(searchParams, (success, error) => {
          ...
          if (success) {
            ...
            success.content.forEach((row) => {
              const actionList = [];
              if(checkPermission("CheckCapitalflowPlatform.get")){
                actionList.push({ key: 'validateFreight', name: '校验运费' });
              }
              if(checkPermission("ReportedCapitalflowPlatform.get")){
                actionList.push({ key: 'reportFreight', name: '上报运费' });
              }
              actionList.push({ key: 'edit', name: '编辑' });
              if(checkPermission("uploadCapitalflowPlatform.delete") && row.reportStatus != 'reported'){

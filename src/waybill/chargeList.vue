<template>
  <div class="charge-list settle-charge clearfix" v-if="chargeItemList.length">
    <div class="page-header clearfix">
      <span class="fr">
        <el-button type="primary" @click="submitRecord" v-show="isEdit">保存</el-button>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    //       console.log('chargeItemList 发生改变：', '新的值为：', newVal, '旧的值为：', oldVal);
    //       if (newVal) {
    //         console.log('chargeItemList 发生改变，开始计算……');
    //         this.$emit('caculate');
    //       }
    //     }
    //   }
...
//          });
      ...
      submitRecord() {
        ...
        this.chargeItemList.forEach((chargeItem) => {
          // 数量或者金额至少填写了一个，才保存该条费用信息记录
          if (chargeItem.chargeItemNumber || chargeItem.chargeAmounts) {
            // 因多条记录参数用逗号拼接提交，为避免提交参数拼接时，出现为空现象导致数据顺序不对，故以下字段未填写时，用占位符占位，
            // eg. 多条记录提交参数为，chargeItemNumber：-,-,1220.0，如果不填写占位符会导致第三条记录显示在第一条
            const notNullFields = ['chargeItemNumber', 'chargeAmounts', 'description', 'attachmentList'];
            notNullFields.forEach((val) => {
              if (!chargeItem[val]) {
                chargeItem[val] = '-';
              }
            });

...
<template>
  <div class="block">
    ...
    <el-dialog title="查看图片" v-model="dialogImgVisible"  custom-class="el-dialog_img">
      <img width="100%" :src="imageUrl" alt="">
    </el-dialog>
    <el-button type="text" @click="showImgDialog" v-if="!isList && !editable && imageUrl">
      <img class="tableImg"  :src="imageUrl" alt="" >
    </el-button>
  </div>
</template>
  ...
  methods: {
    ...
    beforeAvatarUpload(file) {
      const isType = file.type === 'image/jpeg' || file.type === 'image/gif' || file.type === 'image/png',
        ...
        uploadItem = $el.querySelector('.el-upload.el-upload--picture-card');
      if (!isType) {
        this.$message.error('上传附件仅限JPG、GIF、PNG格式!');
        return false;
      }
      if (!isLt3M) {
    ...
    if (params.code && this.domainObject[this.configData.field]) {
      axios.get(this.getImgUrl, {
        ...
        .then((res) => {
          ...
          if (ret.thumbnailList && ret.thumbnailList.length > 0) {
            this.imageUrl = ret.thumbnailList[0].url;
            this.fileList.push({

      ...
      <table class="rl-fix-table table-bordered">
        ...
        <tr>
          <th class="fix-hd fix-col" v-if="selectable">
            ...
            <table-column-set :column-data="columnsData" ref="tablecolumn"  :columnret.sync="showColumnRet" style="float:right" />
          </th>
          <th class="fix-hd fix-col" v-if="operatable">操作</th>
          <!-- <th class="fix-hd fix-col">ID</th> -->
          <th v-for="column in columnsData" v-if="showColumnRet.indexOf(column.fieldConfigCode) > -1" :key="column.name" class="fix-hd">
            {{ column.showName }}
          </th>
        </tr>
        </thead>
        <tbody>
          <tr v-for="(list, index) in listData" :data-error="list.error" :key="index">
            ...
            <td class="fix-col" v-if="operatable">
              <div class="cell" style="width: 100px">
                ...
                <!--<a href="JavaScript:;" data-link="" v-for="action in actionData">-->
                  <!--{{ action.actionName }}-->
                <!--</a>-->
                <!-- Begin actionlst func-->
                <div style="display:none;" v-if="actionLst">{{ flushActionlst(list,index) }}</div>
                <!-- <el-button size="mini" v-if="actionLst && actionLst.length === 1" @click="operationAction(actionLst[0]['actionUrl'], list)">{{ actionLst[0]['name'] }}</el-button>
                <el-dropdown v-else-if="actionLst && actionLst.length > 1" size="mini" split-button>
                  <div @click="operationAction(actionLst[0].actionUrl, list)">{{ actionLst[0].name }}</div>
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-for="(item, index) in actionLst" :key="index" v-if="index > 0">
                      <span @click="operationAction(item.actionUrl, list)">{{ item.name }}</span>
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown> -->
                <el-button size="mini" v-if="operationLst[index] && operationLst[index].length === 1" @click="operationAction(operationLst[index][0]['actionUrl'], list)">{{ operationLst[index][0]['name'] }}</el-button>
                <el-dropdown v-else-if="operationLst[index] && operationLst[index].length > 1" size="mini" split-button  @click="operationAction(operationLst[index][0].actionUrl, list)">
                  {{ operationLst[index][0].name }}
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-for="(item, index) in operationLst[index]" :key="index" v-if="index > 0">
                      <span @click="operationAction(item.actionUrl, list)">{{ item.name }}</span>
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
                ...
                <!-- End actionlst func-->
                <!-- <span v-if="actionLst"></span> -->
                <el-select v-else-if="operValues[list.code]" v-model="operValues[list.code]" placeholder="请选择">
            ...
            <td v-for="(column,index) in columnsData" v-if="showColumnRet.indexOf(column.fieldConfigCode) > -1" :key="index" >
...
export default {
  ...
  methods: {
    flushActionlst(row,index) {
      // console.log('row', row)
      const self = this,
        lst = self.actionLst,
        newLst = []
      lst.forEach((item) => {
        if(item.nodisplayProp) {
          if(item.nodisplay.indexOf(row[item.nodisplayProp]) === -1) {
            if(item.extra){
              if(row[item.extra['key1']] === item.extra['val1']){
                newLst.push(item)
              }
            }else{
              newLst.push(item);
      ...
      self.operationLst[index] = newLst
      // console.log('now operationLst', self.operationLst)
    },
    operationAction(action, item) {
      console.log('operationAction', action)
      const self = this;
      if(action) {
        self.$parent.$emit(`${action}`, item)
      }
    },
    handleDelete(row) {
      const paramObj = {

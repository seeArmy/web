...
* Created by
*/
<!-- 合理货差系数 2019-1-25  在ele-GoodsLossMore 的基础上增加了很多功能 -->
<template>
  <div>
    <!-- radioData.lineFeed !== 'FALSE' -->
    <!-- 说明：当配置了 lineFeed=FALSE 说明该组件不需要换行处理 -->
    <div class="innerblock selectGood">
      ...
      <template v-if="editable && radioData.lineFeed !== 'FALSE' ">
              <br/>
      </template>
      <ele-input v-if=" domainObject[radioData.field] != 'none' ":editable="editable" :configData="inputData" :domainObject="domainObject" :rules="inputDataRule"></ele-input>
      <el-form-item :prop="selectkey" :rules="autoDataRule" v-if="editable && domainObject[radioData.field] != 'none'">
        <el-select v-if="!isList && editable" :disabled="autoData.readonly === 'TRUE'" v-model="domainObject[selectkey]" @change="handleSelect" ref="select" placeholder="">
...
export default {
  ...
  methods: {
    ...
    onRefrshSelect(parm){
      this.codeUnitType = parm.codeUnitType;
      this.changeHandle('goods.loss');
    },

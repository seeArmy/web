...
<script type="text/ecmascript-6">
  ...
  const chinaCity = require('./chinacitygeo.json')
  require('echarts/map/js/china');
  echarts.registerMap('chinacity', JSON.parse(chinaCity));
  ...
  export default {
    ...
    methods: {
      draw() {
        ...
        this.geoLinesChart.showLoading()
        this.geoLinesChart.setOption(option)
        this.geoLinesChart.hideLoading()
      }
    },
    mounted() {
      this.$nextTick(() => {
      ...
      this.__resizeHanlder = debounce(() => {
        if (this.geoLinesChart) {
          this.geoLinesChart.resize();
        }
      },100)

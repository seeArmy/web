...
<template>
  <div class="block">
    <el-form-item :prop="this.configData.field" :rules="rules" v-if="!isList && editable">
      ...
      <el-button v-else class="button-new-tag" size="small" @click="showInput" v-show="configData.hideAdd !== 'TRUE'">添加标签</el-button>
    </el-form-item>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      ...
      //   this.domainObject[this.configData.field] = this.configData.default.split(',');
      // }
      if (this.configData.tagDatasource) {
        DataSourceService[this.configData.tagDatasource].getData({}, (sucess) => {
          if (sucess) {
            console.log(sucess);
            const tempArray = [],
              tempText = [];

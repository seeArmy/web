<template>
  ...
  <div class="pagination">
    ...
    <!-- 跳转 到xx 页  start =============-->
    <span class="page-jump" v-if='isShowPageJump'>
      <input v-model='pageJumpNum' type="number">
      <!-- <button @click="pagerClick(pageJump)">跳转</button> -->
      <el-button type="default" @click="pageJump">跳转</el-button>
    </span>
    <!-- 跳转 到xx 页  end =============-->
  </div>
</template>
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      // 页面跳转
      pageJump(){
        if(this.pageJumpNum>=1 && this.pageJumpNum<= this.totalPage){
          this.pagerClick(Number(this.pageJumpNum));
        }
      },
      ...
      pageSizeChange(event) {
        ...
        this.page = 1;
        this.$emit('change', this.page, newPageSize, 'pageChange');
      },
      cal() {

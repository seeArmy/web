  ...
  <div>
    <ele-list :selectable="selectable" :operatable="operatable"
      ...
      <template slot="funArea">
        <!-- <el-button type="default" v-if="showMenu"   @click="batchUploadWaybill" v-check-permission="'ReportedWaybill.get'">批量上报</el-button> -->
        <el-button type="default"  @click="batchValidWaybill" v-check-permission="'Checktruck.get'">批量校验</el-button>
        <el-button type="default"  @click="deleteValidWaybill" v-check-permission="'uploadWaybill.delete'">批量删除</el-button>  
      </template>
    </ele-list>
...
<script type="text/ecmascript-6">
  ...
  import { getColumn,list,getSearch, downloadTemplate,deleteList, getAction, executeAction, exportCsv, add, downloadErrorData,uploadWaybill,validWaybill } from '../../api/uploadWaybillService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        getSearch,
        objectName: '上报运单',
        addUrl: '/uploadWaybill/add.html',
        editUrl: '/uploadWaybill/add.html',
        listUrl: '/uploadWaybill/list.html',
        actionUrl: 'uploadWaybill/action.html',
          ...
          case 'validWaybill':
            this.$confirm('确认校验本运单吗？', '提示', {confirmButtonText: '确定',cancelButtonText: '取消',type: 'warning'}).then(() => {
              ...
              validWaybill(model.code,(ret,err)=>{
                ...
                else if(ret && ret.content  && false == ret.content.success) this.$message({type: 'success',  message: '校验失败！'});
        ...
        list(searchParams, (success, error) => {
          ...
          if (success) {
            ...
            success.content.forEach((row) => {
              ...
              if(checkPermission("Checktruck.get")){
                actionList.push({ key: 'validWaybill', name: '校验运单' });
              }
              // if(checkPermission("ReportedWaybill.get") && this.showMenu){
              //   actionList.push({ key: 'uploadWaybill', name: '上报运单' });
              // }
              actionList.push({ key: 'edit', name: '编辑' });
              if(checkPermission("uploadWaybill.delete")){

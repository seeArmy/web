...
<script type="text/ecmascript-6">
  ...
  import { add, edit, get } from '../../api/OutputInvoiceService';
  ...
  export default {
    ...
    data() {
      return {
        objectName: '销项发票',
        configUrl: `${ApiConst.apiSettleDomain}/detail_render_info/outputVatInvoice/list`,
        domainObject: {},
        addFun: add,
        editFun: edit,
        getInfo: get,
        uploadUrl: ApiConst.apiSettleDomain,
        logType: 'outputVatInvoice',
    ...
    methods: {
      formMounted(form) {
        // console.log('get field is', form.getElementByField('payerOrgCode'));



        form.getElementByField('billCode').$on('change', (billModel) => {

          console.log('11111',billModel);
          const paraObj = {
            // code: billModel.payerOrgCode
            orgCode: billModel.platformPayerOrgCode
          };
          getOrgInfo(paraObj, (success, error) => {
            ...
            if (success) {
              ...
              this.domainObject.orgNo = orgInfo.orgNo;
              this.domainObject.payerOrgTaxNumber = this.domainObject.payerOrgTaxNumber || orgInfo.taxNumber;
        ...
        form.getElementByField('payerOrgCode').$on('change', (model) => {
          ...
          getOrgInfo(paraObj, (success, error) => {
            ...
            if (success) {
              console.log('partner is', success.content);
              const orgInfo = success.content;
              this.domainObject.payerOrgCode = orgInfo.code;
              this.domainObject.payerOrgTaxNumber = this.domainObject.payerOrgTaxNumber || orgInfo.taxNumber;
              this.domainObject.payerOrgAddress = this.domainObject.payerOrgAddress || orgInfo.address;
              this.domainObject.payerOrgTelephone = this.domainObject.payerOrgTelephone || orgInfo.linkmanPhone;
              this.domainObject.payerOrgBankName = this.domainObject.payerOrgBankName || orgInfo.bankName;
        ...
        getSelfInfo((success, error) => {
          ...
          if (success) {
            ...
            const orgInfo = success.content;
            this.domainObject.payeeOrgName = this.domainObject.payeeOrgName || orgInfo.orgFullName;
            this.domainObject.payeeOrgCode = this.domainObject.payeeOrgCode || orgInfo.code;
            this.domainObject.payeeOrgTaxNumber = this.domainObject.payeeOrgTaxNumber || orgInfo.creditCode;

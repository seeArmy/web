...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, exportCsv, add, downloadErrorData, downloadTemplate } from '../../api/AppVersionService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        getSearch,
        objectName: 'app版本管理',
        addUrl: '/appVersion/add.html',
        editUrl: '/appVersion/add.html',
        listUrl: '/appVersion/list.html',
        deleteFun: deleted,
        exportCsv,
        add,
        downloadErrorData,
        importUrl: `${ApiConst.apiLbsDomain}/import/custom/device`,
        downloadTemplate
      };
    },
    methods: {
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      doRemove(model) {
        ...
        this.$msgbox({
          title: '确认',
          message: `确认要删除版本号 ${model.versionCode}？`,
      ...
      action(command, menu, model) {
        ...
        switch (command) {
          ...
          case 'remove':
            ...
            break;
          case 'copy':
            this.openWindow(`${this.addUrl}?objectName=${this.objectName}&originalCode=${model.code}`, `新建${this.objectName}`);
            break;
          default:
            break;
        }

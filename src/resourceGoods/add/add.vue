...
import { addGoods as ratesAdd, editGoods as ratesEdit, getGoods as ratesGet, listGoodsCategory } from '../../../api/goodsCategoryService';
...
export default {
  ...
  data() {
    return {
      objectName: '货物',
      configUrl: `${ApiConst.apiResourceDomain}/detail_render_info/resourceGoods/list`,
      domainObject: {},
      addFun: ratesAdd,
      editFun: ratesEdit,
      getInfo: ratesGet,
      ...
      logType: 'rates',
      listUrl: '/resourceGoods/list.html',
      listTitle: '货物分类列表',
      status: false,
      code: '',
      fields: [],
  ...
  methods: {
    formReady(formData) {
      ...
      // // 监控计量标准
      this.$watch('domainObject.meterageType', (val) => {
        let ele = this.$refs['form'].getElementByField('goodsPriceUnitCode');
        ele.onRefresh({ codeUnitType: val });
      })
      this.$nextTick(() => {
        let ele = this.$refs['form'].getElementByField('goodsPriceUnitCode');
        ele.onRefresh({ codeUnitType: this.domainObject['meterageType'] });
      })
    },
    editForm() {
      this.editable = true;

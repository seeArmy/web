...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, exportCsv, add, downloadErrorData, downloadTemplate } from '../../api/BankAccountService';
  ...
  export default {
    ...
    data() {
      return {
        ...
        getSearch,
        objectName: '账户信息',
        addUrl: '/bankAccount/add.html',
        editUrl: '/bankAccount/add.html',
        listUrl: '/bankAccount/list.html',
        deleteFun: deleted,
        exportCsv,
        add,
        downloadErrorData,
        importUrl: `${ApiConst.apiPersonDomain}/import/custom/bankAccount`,
        downloadTemplate
      };
    },
    methods: {
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      action(command, menu, model) {
        ...
        switch (command) {
          case 'add':
            if (this.addFrom === 'company') {
              params = `orgCode=${this.params.orgCode}`;
            }
            if (this.addFrom === 'driver') {
              params = `personCode=${this.params.personCode}`;
            }
            this.openWindow(`${this.editUrl}?objectName=${this.objectName}&${params}`, `新建${this.objectName}`);
            break;
          case 'choose':
            this.doChoose(model);
            break;
          case 'edit':
            this.openWindow(`${this.editUrl}?code=${model.code}&objectName=${this.objectName}&editable=true`, `编辑${this.objectName}`);
            break;
          case 'remove':
            this.doRemove(model);
            break;

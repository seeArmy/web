...
<script type="text/ecmascript-6">
  ...
  import { getOutputSummarySearch, getOutputSummaryColumn, getOutputSummaryList, exportOutputSummaryCsv } from '../../api/ReportService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        total: 0,
        loading: true,
        getColumn: getOutputSummaryColumn,
        getList: getOutputSummaryList,
        getSearch: getOutputSummarySearch,
        objectName: '销项统计',
        addUrl: null,
        editUrl: null,
        listUrl: null,
        deleteFun: Function,
        exportCsv: exportOutputSummaryCsv,
        add: Function,
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          getOutputSummaryColumn((success) => {
            const paramsObj = {
              params: params,
              configList: success.content
            };
            paramsObj["params"].obj = 'outputSummary';
            exportOutputSummaryCsv(paramsObj, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        self.listData = [];
        getOutputSummaryList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

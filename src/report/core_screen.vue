  ...
  <div class="info-list-tpl" v-bind:style="styleObject">
    ...
    <div class="info-tab info-tab-open">
      ...
      <div class="cont">
        ...
        <ul class="info-title tc mar_t--15" id="coreScreen">
          ...
          <li class="sep-info">
            <strong class="org">平台车辆数：{{summary.truckCount}}</strong>
            <strong class="org">平台司机数：{{summary.driverCount}}</strong>
          </li>
        </ul>
        <dl class="info-list mar_t--15">
          ...
          <dd v-for="(line, index) in lineTop5">
            ...
            <span :title="[line.loadingProvince + line.loadingCity + line.loadingCounty + '——' + line.unloadingProvince + line.unloadingCity + line.unloadingCounty]">{{ line.loadingCity }} {{ line.loadingCounty }} —— {{ line.unloadingCity }} {{ line.unloadingCounty }}</span>
            <span class="fr">{{ line.showValue }} {{ line.unit }}</span>
          </dd>
        </dl>
...
<script type="text/ecmascript-6">
  ...
  import { screenSummary, screenPlatformInfoSummary, screenLineSummary, screenGoodsSummary, screenTransportLive } from '../../api/ReportService';
  import { dateFormat } from '../../api/Utils';
  import { geoCountryMap } from '../../api/EChartUtil';
  ...
  export default {
    ...
    methods: {
      ...
      renderScreenSummary(params) {
        ...
        screenPlatformInfoSummary({}, (success) => {
          const content = success.content;
          this.summary.truckCount = content.truckCount;
          this.summary.driverCount = content.driverCount;
        });
      },
      renderScreenLineSummary(params) {
        screenLineSummary(params, (success) => {
          const list = success.content || [],
            ...
            coordMap = geoCountryMap(),
            geoLines = [];
          let lineList = [];
          list.forEach((line) => {
            if (coordMap[line.loadingCounty] && coordMap[line.unloadingCounty]) {
              const w = this.tenThousandFormat(line.weight),
                v = this.tenThousandFormat(line.volume),
                n = this.tenThousandFormat(line.num),
                corrds = [];
              ton.push({ loadingCounty: line.loadingCounty, loadingCity: line.loadingCity, loadingProvince: line.loadingProvince,  unloadingCounty: line.unloadingCounty, unloadingCity: line.unloadingCity, unloadingProvince: line.unloadingProvince, value: line.weight, showValue: w, unit: '吨' });
              cube.push({ loadingCounty: line.loadingCounty, loadingCity: line.loadingCity, loadingProvince: line.loadingProvince, unloadingCounty: line.unloadingCounty, unloadingCity: line.unloadingCity, unloadingProvince: line.unloadingProvince, value: line.volume, showValue: v,unit: '方' });
              item.push({ loadingCounty: line.loadingCounty, loadingCity: line.loadingCity, loadingProvince: line.loadingProvince, unloadingCounty: line.unloadingCounty, unloadingCity: line.unloadingCity, unloadingProvince: line.unloadingProvince, value: line.num, showValue: n, unit: '件' });
              corrds.push(coordMap[line.loadingCounty]);
              corrds.push(coordMap[line.unloadingCounty]);
              geoLines.push({ fromName: line.loadingProvince + line.loadingCity + line.loadingCounty, toName: line.unloadingProvince + line.unloadingCity + line.unloadingCounty, coords: corrds, weight: w, volume: v, num: n });
      ...
      handleUnit(unit) {
        const that = this,
          params = {
            from_unloadingTime: that.startDate,
            to_unloadingTime: that.endDate
          };
        that.type = unit;

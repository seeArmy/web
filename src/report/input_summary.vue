...
<script type="text/ecmascript-6">
  ...
  import { getInputSummarySearch, getInputSummaryColumn, getInputSummaryList, exportInputSummaryCsv } from '../../api/ReportService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        total: 0,
        loading: true,
        getColumn: getInputSummaryColumn,
        getList: getInputSummaryList,
        getSearch: getInputSummarySearch,
        objectName: '进项统计',
        addUrl: null,
        editUrl: null,
        listUrl: null,
        deleteFun: Function,
        exportCsv: exportInputSummaryCsv,
        add: Function,
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      exportData(params) {
        ...
        if (dataTotal > 10000) {
          ...
          getInputSummaryColumn((success) => {
            const paramsObj = {
              params: params,
              configList: success.content
            };
            paramsObj["params"].obj = 'inputSummary';
            exportInputSummaryCsv(paramsObj, (success, error) => {
      ...
      query(searchParams) {
        ...
        this.loading = true;
        self.listData = [];
        getInputSummaryList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

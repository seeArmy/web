        ...
        <div class="inner clearfix">
          ...
          <div class="regform">
            <div class="reghd clearfix">
              <h2 class="logtit fl">账号激活<em>ACCOUNT ACTIVATION</em></h2><p class="lognow fr">已有账号？直接去 <a href="/login/login.html">登录</a></p>
            </div>
            <div class="steps">
              <ul>
                <li class="step1 focus">1、填写信息</li>
                <li class="step2">2、提交激活</li>
            ...
            <div class="regbd">
              <div id="userForm">
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="reg-form formlist">
                  ...
                  <el-form-item>
                    <el-button type="primary" @click="submitForm('ruleForm')" :disabled="btnDisabled">提交</el-button>
                  </el-form-item>
                </el-form>
              </div>
            </div>
          ...
          if (value.length === 11) {
            if (reg.test(value) === false) {
              ...
              registerService.checkUser(paramObj, (success) => {
                if (success) {
                  EmployeeService.validatePhone({ phone: value }, (res) => {
                    const exist = res.content.result;
                    if (!exist) {
                      callback(new Error('手机号码不存在，请联系管理员添加后再激活！'));
                    } else {
                      callback();
    ...
    methods: {
      ...
      confirmSubmit() {
        ...
        registerService.addUser(this.ruleForm, (success, error) => {
          if (success) {
            const loginAccount = success.content.loginAccount;
            EmployeeService.registerList({ size: 100, phone: loginAccount }, (res) => {
              const listLength = res.content.length;
              if (listLength > 1) {
                window.location.href = `/register/activation.html?loginAccount=${loginAccount}`;
              } else if (listLength === 1) {
                this.code = res.content[0].code;
                this.paramObj.ownerOrgName = res.content[0].ownerOrgName;
                this.paramObj.ownerOrgCode = res.content[0].ownerOrgCode;
                this.paramObj.fullName = res.content[0].fullName;
                PlatformService.setUserInfoCode(this.code, this.paramObj, (respond, err) => {
                  if (err || respond.code !== 200) {
                    this.$message({
                      type: 'error',
                      message: err.content,
                      duration: 5000

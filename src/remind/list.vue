...
<script type="text/ecmascript-6">
  ...
  import { getColumn, list, getSearch, deleted, exportCsv, add, downloadErrorData, downloadTemplate } from '../../api/RemindService';
  ...
  export default {
    ...
    data: () => {
      return {
        ...
        getSearch,
        objectName: '待办提醒',
        listUrl: '/remind/list.html',
        addUrl: '/remind/add.html',
        editUrl: '/remind/add.html',
        deleteFun: deleted,
        exportCsv,
        add,
        downloadErrorData,
        importUrl: `${ApiConst.apiMaintenaceDomain}/import/custom/remind`,
        downloadTemplate
      };
    },
    methods: {
...
//        console.log('more', `${this.listUrl}?objectName=${this.objectName}&${this.toParameterArray(model).join('&')}`);
      ...
      doRemove(model) {
        ...
        this.$msgbox({
          title: '确认',
          message: `确认要删除${model.truckLicenseNo}待办提醒吗？`,

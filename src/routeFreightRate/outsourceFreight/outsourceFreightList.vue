...
<template>
  <div id="clientFreightList">
    ...
    <p class='underline'>外包运价单号：{{tempDomainObject.ratesSystemNo}}</p>
    <p class='text_center'>确定申请审核这笔外包运价单吗？</p>
    ...
    <p style='margin: 20px 0 10px;'>共选中{{checkNum}}条，您确定申请审核这些外包运价单吗？</p>
...
import { getOutsourcingColumn,getOutsourcingSearch,getOutsourcingList,applyOutsourcingList,checkutsourcingList,removeOutsourcingList,endOutsourcingList} from '../../../api/routeFreightRateService';
...
export default {
  ...
  data: () => {
    return {
      ...
      total: 0,
      loading: true,
      getColumn:getOutsourcingColumn,
    ...
    //   actionExecuteFun: executeAction,
      ...
      getList:getOutsourcingList,
      getSearch:getOutsourcingSearch,
      objectName: '外包运价单',
      addUrl: '/routeFreightRate/outsourceFreight/outsourceFreightAdd.html',
      editUrl: '/routeFreightRate/outsourceFreight/outsourceFreightAdd.html',
      recycleUrl: '',
      listUrl: '',
      actionUrl: '',
      importUrl: ``,
  ...
  methods: {
    ...
    batchApplica(){
      // this.dialogVisible.batchApplicatio=true;
       const that = this,
        ...
        this.$msgbox({
          title: '批量申请运价',
          message: `共选中${list.length}条，确定申请这些外包运价单吗?`,
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              list.forEach((item) => {item
                applyOutsourcingList({code:item}, (success, error) => {
    ...
    batchAuditColse(){
      ...
      const that = this,
        ...
        list.forEach((item) => {
          ...
          checkutsourcingList(params, (success, error) => {
            that.batchApplyNum -= 1;
            if (that.batchApplyNum === 0) {
              this.$message({
                type: 'success',
    ...
    // 申请运价apply
    apply(){
      applyOutsourcingList({code:this.tempDomainObject.code},(success, error) => {
    ...
    //结束运价
      ...
      endOutsourcingList({code:this.tempDomainObject.code},(success, error) => {
          if (success && success.code ==200) {
            this.$message({
              type: 'success',
              message: '提交结束成功',
    ...
    //删除运价
    remove(){
        removeOutsourcingList({code:this.tempDomainObject.code},(success, error) => {
          if (success && success.code==200) {
            this.$message({
              type: 'success',
              message: '提交删除成功',
    ...
    query(searchParams) {
      console.log('searchParams', searchParams);
        ...
        this.loading = true;
        this.listData = [];
        getOutsourcingList(searchParams, (success, error) => {
          if (error) {
            this.$message({
              type: 'error',
              message: error.content,

  ...
  <div class="invoice-section">
    ...
    <div class="invoice-info">
      <div class="hd-opr clearfix">
        <el-button class="fr" v-if="domainObject.invoiced === 'no' " @click="confirmInvoiced">确认已全部开票</el-button>
      </div>
      <div class="bill-info">
        <ul class="bi-list clearfix">
          ...
          <li>
            ...
            <div class="bi-cont">
              <strong class="org">
                {{ domainObject.remainderInvoiceAmount }}
              </strong>
            </div>
          </li>
          <li>
            ...
            <div class="bi-cont">
              <strong>
                  {{ domainObject.invoiced === 'yes' ? '已收票' : '未收票' }}
              </strong>
            </div>
          </li>
        </ul>
    ...
    <!-- section 2 -->
    <div class="hd-opr clearfix">
      <el-button class="fr" @click="openWindow()">新建销项发票</el-button>
    </div>
    <div class="invoice-card" v-for="item in invoiceData" :key="item.code">
      <ul class="card-info clearfix">
        ...
        <li>
          ...
          <div class="bi-cont">
            <strong>
              {{ item.invoiceTime | filterInvoiceTime}}
            </strong>
          </div>
        </li>
        <li>
...
<script>
  import { settleReceivableInvoice } from '../../api/SettleBillService';
  import { list } from '../../api/OutputInvoiceService';
    ...
    methods: {
      openWindow(code) {
        let url = '',
          tit = '';
        if (code) {
          url = `/outputInvoice/add.html?code=${code}&editable=false`;
          tit = '销项发票详情';
        } else {
          url = `/outputInvoice/add.html?objectName=销项发票&billCode=${this.domainObject.code}`;
          tit = '新建销项发票';
        }
        urlRedirect(url, tit);
      },
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              settleReceivableInvoice(self.code, (success, error) => {
                if (error) {
                  this.$message({
                    type: 'error',

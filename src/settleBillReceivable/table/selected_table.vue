  ...
  <div class="uc-content table2">
    <el-form :inline="true" :model="searchModel" ref="searchModel" class="demo-form-inline">
      ...
      <div class="search-button">
        <el-button type="primary" @click="onSubmit"><i class="el-icon-search"></i> 已选内搜索</el-button>
...
<script>
  ...
  import { settleReceivableSelectSelectedWaybillList, settleReceivableWaybillColumns, settleReceivableWaybillSearch, settleReceivableClearSelectedWaybill, settleReceivableWaybillRemove } from '../../../api/SettleBillService';
  ...
  export default {
    ...
    methods: {
          ...
          this.$msgbox({
            ...
            beforeClose: (action, instance, done) => {
              if (action === 'confirm') {
                ...
                settleReceivableWaybillRemove(params, (success, error) => {
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              instance.confirmButtonLoading = true;
              instance.confirmButtonText = '执行中...';
              settleReceivableClearSelectedWaybill(params, (success, error) => {
                this.$refs.eleTable.batchSelect = false;
                this.getData();
                if (success) {
                  this.$message({
      ...
      getData() {
        ...
        settleReceivableSelectSelectedWaybillList(this.paramObj, (success, error) => {
          if (error) {
            that.loading = false;
            this.$message({
              type: 'error',
    ...
    created() {
      ...
      settleReceivableWaybillSearch((searchConfig) => {
        if (searchConfig) {
          ...
          // self.miniFields = paramData.field.slice(0, 3);

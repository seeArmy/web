  ...
  methods: {
    getData() {
      const params = getParamsFromURL(window.location.search),
        apiDomain = params.domain || this.domain,
        apiType = params.type || this.type;
      let apiCode = params.code || params.truckCode || this.code;

      // 如果直接是传进来的，那么直接用
      if (this.params && this.params.truckCode) {
        apiCode = this.params.truckCode;
      }
      // console.log('---------------------------------------------', params);
          ...
          if (success) {
            ...
            tempData.forEach((val) => {
              ...
              Object.keys(textMap).forEach((key) => {
                ...
                let flag = (key == '修改时间') || (key == '客户运价' && !checkPermission('client.price.show'));
                if(!flag){
                   textArray.push(text);
                }
              });

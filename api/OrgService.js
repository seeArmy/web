...
function editBank(paramObj, success) {
  const apiUrl = `${apiDomain}/org/self/editBank`;
  HTTP.post(apiUrl, paramObj, (response, err) => {
    success(response, err);
  });
}

function verify(paramObj, success) {
  const apiUrl = `${apiDomain}/org/verify`;
  HTTP.post(apiUrl, paramObj, (response, err) => {
    success(response, err);
  });
}
...
function getDetail(cb) {
  var success = require(`./fieldMetadata/${ApiConst.fieldMetadataDir}/org/company/detail_render_info.json`)
 success = JSON.parse(JSON.stringify(success));
    cb(success);
}

export { add, applyCert, selfs, edit, editBank, verify, personCancel, personConfirm, get, apply, addDept, editDept, deletedDept, getSelfDept, getDetail };

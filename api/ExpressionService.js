...
function add(paramObj, callback) {
  const apiUrl = `${apiDomain}/expression/add`;
...
function edit(code, paramObj, callback) {
  const apiUrl = `${apiDomain}/expression/edit`;
  HTTP.post(apiUrl, paramObj, (success, error) => {
    callback(success, error);
  });
}
...
function list(paramObj, callback) {
  const apiUrl = `${apiDomain}/expression/list`;
...
function deleted(paramObj, callback) {
  const apiUrl = `${apiDomain}/expression/delete/${paramObj.code}/`;
...
function get(paramObj, callback) {
  const apiUrl = `${apiDomain}/expression/detail/${paramObj.code}/`;
    ...
    content: [
      {
        ...
        elementCode: 'label',
        extraParams: '{"field1": "sort", "detailLinkVisiable": "true", "required": "false"}'
      },
      {
        showName: '公式名称',
        fieldConfigCode: 'expressionName',
        elementCode: 'label',
        extraParams: '{"field1": "expressionName", "detailLinkVisiable": "true", "required": "false"}'
      },
      {
        showName: '公式代码',

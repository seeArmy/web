...
function getPermissionList(callback) {
  const url = `${apiDomain}/permission/person/resource/list/get`;
  HTTP.get(url, {}, (success, error) => {
    callback(success, error);
  });
}
...
function getRolePermission(paramObj, callback) {
  const url = `${apiDomain}/permission/person/resource/${paramObj.code}/list`;
  HTTP.get(url, {}, (success, error) => {
    callback(success, error);
  });
}
...
export { getMember, getPermission, getPermissionList, getRolePermission, editUserInfo, savePermissionEdit, getUserRoleList };

...
function validatePhone(paramObj, cb) {
  const apiUrl = `${apiDomain}/employee/validate/phone`;
  HTTP.get(apiUrl, paramObj, (response, error) => {
    cb(response, error);
  });
}
...
function getColumn(cb) {
  var success = require(`./fieldMetadata/${ApiConst.fieldMetadataDir}/person/employee/list_render_info.json`)
...
function getSearch(cb) {
  var success = require(`./fieldMetadata/${ApiConst.fieldMetadataDir}/person/employee/search_render_info.json`)
 success = JSON.parse(JSON.stringify(success));
    cb(success);
}
 ...
 * @param callback
 */
function quit(paramObj, cb) {
  const apiUrl = `${apiDomain}/person/exit_org`;
  HTTP.post(apiUrl, paramObj, (success, error) => {
    cb(success, error);
  });
}
 ...
 * @param callback
 */
function switchOrg(paramObj, cb) {
  const apiUrl = `${apiDomain}/person/switching_org`;
  HTTP.post(apiUrl, paramObj, (success, error) => {
    cb(success, error);
  });
}
...
function exportCsv(paramObj, callback) {
  const apiUrl = `${apiDomain}/export/custom/employee`;
...
export { list, registerList, getColumn, getSearch, add, edit, applyOrg, getPhone, deleted, quit, switchOrg, disable, enable, get, getRole, getDept, editRole, exportCsv, downloadErrorData, downloadTemplate, personDeptRelationList, validatePhone };

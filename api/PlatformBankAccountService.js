...
function add(paramObj, callback) {
  const apiUrl = `${apiDomain}/person/bank/account/add`;
...
function edit(code, paramObj, callback) {
  const apiUrl = `${apiDomain}/person/bank/account/edit`;
  HTTP.post(apiUrl, paramObj, (success, error) => {
    callback(success, error);
  });
}
...
function list(paramObj, callback) {
  const apiUrl = `${apiDomain}/platform/driver/bank/account/list`;
...
function deleted(paramObj, callback) {
  const apiUrl = `${apiDomain}/person/bank/account/${paramObj.code}/delete`;
...
function get(paramObj, callback) {
  const apiUrl = `${apiDomain}/person/bank/account/${paramObj.code}`;
...
// 占位，主要是为了不报错
function exportCsv(id, callback) {
   callback(null, null);
}

function downloadErrorData(id, callback) {
   callback(null, null);
}
function downloadTemplate(id, callback) {
   callback(null, null);
}
